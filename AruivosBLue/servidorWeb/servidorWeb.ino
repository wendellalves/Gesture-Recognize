...
void setup()
{
  ...
  WiFi.begin("LAR", "LAR@1480");//Conecta no WiFi (COLOQUE O NOME E SENHA DA SUA REDE!).
  ...
  pinMode(D2, OUTPUT);//Define o LED_BUILTIN como Saida.
}
...
void http()//Sub rotina que verifica novos clientes e se sim, envia o HTML.
{
  ...
  if (cliente == true)//Se houver clientes conectados, ira enviar o HTML.
  {
    ...
    {
      digitalWrite(D2, !digitalRead(D2));//Inverte o estado do led.
    }
